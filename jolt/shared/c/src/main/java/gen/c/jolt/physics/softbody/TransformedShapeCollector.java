/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package gen.c.jolt.physics.softbody;

import com.github.xpenatan.jParser.api.NativeObject;
import gen.c.jolt.physics.collision.TransformedShape;
import gen.c.jolt.physics.body.Body;

public class TransformedShapeCollector extends NativeObject {

    private TransformedShape TransformedShape_TEMP_GEN_0;

    static private Body Body_TEMP_STATIC_GEN_0;

    static private TransformedShape TransformedShape_TEMP_STATIC_GEN_0;

    private static final java.util.ArrayList<TransformedShapeCollector> TEAVMC_CALLBACKS = new java.util.ArrayList<>();

    private int teavmcCallbackId = -1;

    private int teavmcRegisterCallback() {
        if (teavmcCallbackId < 0) {
            teavmcCallbackId = TEAVMC_CALLBACKS.size();
            TEAVMC_CALLBACKS.add(this);
        } else {
            TEAVMC_CALLBACKS.set(teavmcCallbackId, this);
        }
        return teavmcCallbackId;
    }

    private static abstract class TEAVMC_Reset_Function extends org.teavm.interop.Function {

        public abstract void call(int callbackId);
    }

    @org.teavm.interop.Export(name = "teavmc_TransformedShapeCollector_Reset")
    private static void teavmc_TransformedShapeCollector_Reset(int callbackId) {
        TEAVMC_CALLBACKS.get(callbackId).internal_Reset();
    }

    private static abstract class TEAVMC_OnBody_Function extends org.teavm.interop.Function {

        public abstract void call(int callbackId, long inBody_addr);
    }

    @org.teavm.interop.Export(name = "teavmc_TransformedShapeCollector_OnBody")
    private static void teavmc_TransformedShapeCollector_OnBody(int callbackId, long inBody_addr) {
        TEAVMC_CALLBACKS.get(callbackId).internal_OnBody(inBody_addr);
    }

    private static abstract class TEAVMC_AddHit_Function extends org.teavm.interop.Function {

        public abstract void call(int callbackId, long inResult_addr);
    }

    @org.teavm.interop.Export(name = "teavmc_TransformedShapeCollector_AddHit")
    private static void teavmc_TransformedShapeCollector_AddHit(int callbackId, long inResult_addr) {
        TEAVMC_CALLBACKS.get(callbackId).internal_AddHit(inResult_addr);
    }

    @org.teavm.interop.Import(name = "teavmc_TransformedShapeCollector_setupCallback")
    private static native void setupCallback(long this_addr, int callbackId, TEAVMC_Reset_Function Reset_fp, TEAVMC_OnBody_Function OnBody_fp, TEAVMC_AddHit_Function AddHit_fp);

    static public final TransformedShapeCollector NULL = TransformedShapeCollector.native_new();

    /**
     * Dummy constructor, used internally to creates objects without C++ pointer
     */
    @Deprecated()
    protected TransformedShapeCollector(byte b, char c) {
    }

    /**
     * @return An empty instance without a native address
     */
    public static TransformedShapeCollector native_new() {
        return new TransformedShapeCollector((byte) 0, (char) 0);
    }

    protected void deleteNative() {
        internal_native_deleteNative(native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_deletenative")
    public static native void internal_native_deleteNative(long this_addr);

    public void ResetNative() {
        internal_native_ResetNative(native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_resetnative")
    public static native void internal_native_ResetNative(long this_addr);

    public void SetContext(TransformedShape inContext) {
        internal_native_SetContext(native_address, inContext.native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_setcontext")
    public static native void internal_native_SetContext(long this_addr, long inContext_addr);

    public TransformedShape GetContext() {
        long addr = internal_native_GetContext_addr(native_address);
        if (addr == 0)
            return TransformedShape.NULL;
        if (TransformedShape_TEMP_GEN_0 == null)
            TransformedShape_TEMP_GEN_0 = TransformedShape.native_new();
        TransformedShape_TEMP_GEN_0.internal_reset(addr, false);
        return TransformedShape_TEMP_GEN_0;
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_getcontext_addr")
    public static native long internal_native_GetContext_addr(long this_addr);

    public void UpdateEarlyOutFraction(float inFraction) {
        internal_native_UpdateEarlyOutFraction(native_address, inFraction);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_updateearlyoutfraction")
    public static native void internal_native_UpdateEarlyOutFraction(long this_addr, float inFraction);

    public void ResetEarlyOutFraction(float inFraction) {
        internal_native_ResetEarlyOutFraction(native_address, inFraction);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_resetearlyoutfraction_l_f")
    public static native void internal_native_ResetEarlyOutFraction(long this_addr, float inFraction);

    public void ResetEarlyOutFraction() {
        internal_native_ResetEarlyOutFraction(native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_resetearlyoutfraction_l")
    public static native void internal_native_ResetEarlyOutFraction(long this_addr);

    public void ForceEarlyOut() {
        internal_native_ForceEarlyOut(native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_forceearlyout")
    public static native void internal_native_ForceEarlyOut(long this_addr);

    public boolean ShouldEarlyOut() {
        return internal_native_ShouldEarlyOut(native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_shouldearlyout")
    public static native boolean internal_native_ShouldEarlyOut(long this_addr);

    public float GetEarlyOutFraction() {
        return internal_native_GetEarlyOutFraction(native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_getearlyoutfraction")
    public static native float internal_native_GetEarlyOutFraction(long this_addr);

    public float GetPositiveEarlyOutFraction() {
        return internal_native_GetPositiveEarlyOutFraction(native_address);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_getpositiveearlyoutfraction")
    public static native float internal_native_GetPositiveEarlyOutFraction(long this_addr);

    public TransformedShapeCollector() {
        long addr = internal_native_create_addr();
        internal_reset(addr, true);
        setupCallback();
    }

    private void setupCallback() {
        int callbackId = teavmcRegisterCallback();
        setupCallback(native_address, callbackId, org.teavm.interop.Function.get(TEAVMC_Reset_Function.class, TransformedShapeCollector.class, "teavmc_TransformedShapeCollector_Reset"), org.teavm.interop.Function.get(TEAVMC_OnBody_Function.class, TransformedShapeCollector.class, "teavmc_TransformedShapeCollector_OnBody"), org.teavm.interop.Function.get(TEAVMC_AddHit_Function.class, TransformedShapeCollector.class, "teavmc_TransformedShapeCollector_AddHit"));
    }

    protected void Reset() {
    }

    private void internal_Reset() {
        Reset();
    }

    protected void OnBody(Body inBody) {
    }

    private void internal_OnBody(long inBody_addr) {
        if (Body_TEMP_STATIC_GEN_0 == null)
            Body_TEMP_STATIC_GEN_0 = Body.native_new();
        Body_TEMP_STATIC_GEN_0.internal_reset(inBody_addr, false);
        OnBody(Body_TEMP_STATIC_GEN_0);
    }

    protected void AddHit(TransformedShape inResult) {
    }

    private void internal_AddHit(long inResult_addr) {
        if (TransformedShape_TEMP_STATIC_GEN_0 == null)
            TransformedShape_TEMP_STATIC_GEN_0 = TransformedShape.native_new();
        TransformedShape_TEMP_STATIC_GEN_0.internal_reset(inResult_addr, false);
        AddHit(TransformedShape_TEMP_STATIC_GEN_0);
    }

    @org.teavm.interop.Import(name = "jolt_physics_softbody_transformedshapecollector_create_addr")
    public static native long internal_native_create_addr();
}
