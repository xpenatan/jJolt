/*-------------------------------------------------------
 * This file was generated by JParser
 *
 * Do not make changes to this file
 *-------------------------------------------------------*/

package jolt.enums;

import com.github.xpenatan.jParser.api.NativeEnum;

public enum ESwingType implements NativeEnum<ESwingType> {

    CUSTOM(0), Cone(ESwingType_Cone_NATIVE()), Pyramid(ESwingType_Pyramid_NATIVE());

    private int value;

    private ESwingType(int value) {
        this.value = value;
    }

    public int getValue() {
        return value;
    }

    public ESwingType setValue(int value) {
        if (this != CUSTOM) {
            throw new RuntimeException("Cannot change none CUSTOM value");
        }
        this.value = value;
        return this;
    }

    public ESwingType getCustom() {
        return CUSTOM;
    }

    private static int ESwingType_Cone_NATIVE() {
        return jolt.enums.natives.JNI_ESwingType.ESwingType_Cone_NATIVE();
    }

    private static int ESwingType_Pyramid_NATIVE() {
        return jolt.enums.natives.JNI_ESwingType.ESwingType_Pyramid_NATIVE();
    }
}
