...
dependencies {
    ...
    if(LibExt.useRepoLibs) {
        implementation("com.github.xpenatan.xJolt:jolt-web:${LibExt.exampleVersion}")
    }
    else {
        implementation(project(":jolt:jolt-web"))
    }
...
libfdx {
    assets(rootProject.file("examples/samples/assets"))
    js {
        mainClass.set("jolt.example.samples.app.web.JoltWebJsLauncher")
        htmlTitle.set("xJolt libfdx - WebGL JS")
    ...
    wasm {
        ...
        htmlTitle.set("xJolt libfdx - WebGL Wasm")
        canvasId.set("libfdx-canvas")
        htmlWidth.set(0)
        htmlHeight.set(0)
    }
    ...
    return tasks.register(taskName) {
        ...
        doLast {
            ...
            if(missing.isNotEmpty()) {
                throw GradleException("Missing xJolt web runtime scripts: ${missing.joinToString()}")
            }
        }
    }
}
...
tasks.register("jolt_sample_webgl_js_build") {
    group = "application"
    description = "Builds the xJolt libfdx WebGL JavaScript sample."
    dependsOn("libfdx_web_js_build", copyJoltJsRuntimeScripts)
}

tasks.register("jolt_sample_webgl_wasm_build") {
    group = "application"
    description = "Builds the xJolt libfdx WebGL Wasm sample."
    dependsOn("libfdx_web_wasm_build", copyJoltWasmRuntimeScripts)
}

tasks.register("jolt_sample_webgpu_js_build") {
    group = "application"
    description = "Builds the xJolt libfdx WebGPU JavaScript sample."
    dependsOn("libfdx_web_js_build", copyJoltJsRuntimeScripts)
    configureWebGpuPage("dist/web-js/webapp", "xJolt libfdx - WebGPU JS")
}

tasks.register("jolt_sample_webgpu_wasm_build") {
    group = "application"
    description = "Builds the xJolt libfdx WebGPU Wasm sample."
    dependsOn("libfdx_web_wasm_build", copyJoltWasmRuntimeScripts)
    configureWebGpuPage("dist/web-wasm/webapp", "xJolt libfdx - WebGPU Wasm")
}

tasks.register<io.github.libfdx.gradle.LibfdxRunWebTask>("jolt_sample_webgl_js_run") {
    group = "application"
    description = "Builds and serves the xJolt libfdx WebGL JavaScript sample."
...
tasks.register<io.github.libfdx.gradle.LibfdxRunWebTask>("jolt_sample_webgl_wasm_run") {
    group = "application"
    description = "Builds and serves the xJolt libfdx WebGL Wasm sample."
...
tasks.register<io.github.libfdx.gradle.LibfdxRunWebTask>("jolt_sample_webgpu_js_run") {
    group = "application"
    description = "Builds and serves the xJolt libfdx WebGPU JavaScript sample."
...
tasks.register<io.github.libfdx.gradle.LibfdxRunWebTask>("jolt_sample_webgpu_wasm_run") {
    group = "application"
    description = "Builds and serves the xJolt libfdx WebGPU Wasm sample."
