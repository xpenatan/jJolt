...
public final class JoltDesktopLauncher {
    ...
    public static void main(String[] args) {
        String graphics = option(args, "--graphics=", System.getProperty("xjolt.sample.graphics", "wgpu"));
        String label = option(args, "--graphics-label=", System.getProperty("xjolt.sample.graphicsLabel"));
        boolean visible = Boolean.parseBoolean(option(args, "--visible=",
                System.getProperty("xjolt.sample.visible", "true")));
        long exitAfterFrames = Long.parseLong(option(args, "--exit-after-frames=",
                System.getProperty("xjolt.sample.exitAfterFrames", "0")));
        DesktopApplicationConfig config = new DesktopApplicationConfig()
                .title("xJolt libfdx - " + graphicsDisplayName(graphics, label))
                .size(960, 540)
                .visible(visible)
                .vSync(true)
                .foregroundFps(60)
        ...
        new DesktopApplicationBackend().start(config, new JoltSampleApplication(exitAfterFrames));
    }
